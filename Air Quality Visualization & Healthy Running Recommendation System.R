...
running_spots <- data.frame(
  ...
  # Reverted back to Tsim Sha Tsui as requested
...
get_advice <- function(aqhi) {
  ...
  # Logic: If value is 0 or NA, treat as No Data
  if (is.na(val) || val == 0) {
...
server <- function(input, output, session) {
    ...
    res_list <- lapply(1:nrow(df), function(i) {
      ...
      if (!is.null(aqhi_df)) {
        ...
        if (length(match_idx) > 0) {
          current_aqhi <- aqhi_df$aqhi[match_idx[1]]
        }
      }
  ...
  output$map <- renderLeaflet({
    ...
    icons <- awesomeIcons(
      icon = "star",
      library = "fa",
      markerColor = sapply(df$AQHI, getColor)
    )
  ...
  output$spots_table <- renderDT({
    ...
    df_show <- processed_data() %>% 
      select(Name, Distance, AQHI, Advice) %>%
      mutate(AQHI = ifelse(AQHI == 0, "No Data", as.character(AQHI))) %>% 
      rename("Running Spot" = Name, "Dist (km)" = Distance)
